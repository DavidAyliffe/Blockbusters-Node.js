...
exports.getAll = async () => {
  const [rows] = await db.query("SELECT payment_id, customer_id, staff_id, rental_id, amount, payment_date, last_update FROM rental");
  return rows;
};

exports.getById = async (id) => {
  const [rows] = await db.query("SELECT payment_id, customer_id, staff_id, rental_id, amount, payment_date, last_update FROM rental WHERE actor_id = ?", [id] );
